...
from util.deformation import generate_voxel_coordinate_grid
...
def calculate_jacobian_determinants(ddf: Tensor) -> Mapping[str, Tensor]:
    ...
    kernels = {}
    kernels["D0x"] = tensor([-0.5, 0, 0.5], dtype=trans.dtype, device=trans.device).view(
        1, 1, 3, 1, 1
    )
    kernels["D+x"] = tensor([0, -1, 1], dtype=trans.dtype, device=trans.device).view(1, 1, 3, 1, 1)
    kernels["D-x"] = tensor([-1, 1, 0], dtype=trans.dtype, device=trans.device).view(1, 1, 3, 1, 1)

    kernels["D0y"] = tensor([-0.5, 0, 0.5], dtype=trans.dtype, device=trans.device).view(
        1, 1, 1, 3, 1
    )
    kernels["D+y"] = tensor([0, -1, 1], dtype=trans.dtype, device=trans.device).view(1, 1, 1, 3, 1)
    kernels["D-y"] = tensor([-1, 1, 0], dtype=trans.dtype, device=trans.device).view(1, 1, 1, 3, 1)

    kernels["D0z"] = tensor([-0.5, 0, 0.5], dtype=trans.dtype, device=trans.device).view(
        1, 1, 1, 1, 3
    )
    kernels["D+z"] = tensor([0, -1, 1], dtype=trans.dtype, device=trans.device).view(1, 1, 1, 1, 3)
    kernels["D-z"] = tensor([-1, 1, 0], dtype=trans.dtype, device=trans.device).view(1, 1, 1, 1, 3)

    # J1*
    kernels["1*xy"] = tensor(
        [[1, 0, 0], [0, -1, 0], [0, 0, 0]], dtype=trans.dtype, device=trans.device
    ).reshape(1, 1, 3, 3, 1)
    kernels["1*xz"] = tensor(
        [[1, 0, 0], [0, -1, 0], [0, 0, 0]], dtype=trans.dtype, device=trans.device
    ).reshape(1, 1, 3, 1, 3)
    ...
    # J2*
    kernels["2*xy"] = tensor(
        [[0, 0, 0], [0, -1, 0], [0, 0, 1]], dtype=trans.dtype, device=trans.device
    ).reshape(1, 1, 3, 3, 1)
    kernels["2*xz"] = tensor(
        [[0, 0, 0], [0, -1, 0], [0, 0, 1]], dtype=trans.dtype, device=trans.device
    ).reshape(1, 1, 3, 1, 3)
...
def calculate_non_diffeomorphic_volume_map(
    ...
    for diff_direction in ["+++", "++-", "+-+", "+--", "j1*", "j2*", "-++", "-+-", "--+", "---"]:
        ...
        non_diff_volume_map = (
            volume_map if non_diff_volume_map is None else non_diff_volume_map + volume_map
        )
